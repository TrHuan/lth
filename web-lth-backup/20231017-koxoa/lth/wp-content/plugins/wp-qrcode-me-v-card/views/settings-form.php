...
<table class="form-table" role="presentation">
    ...
    <tr class="warn-message" <?php echo ( ! empty( $wqm_fgcolor ) or ! empty( $wqm_bgcolor ) ) ? '' : 'style="display:none;"' ?>>
        <th></th>
        <td>
            <div class="please-warn"><p><?php _e( 'Please! Make sure that you colored qr-code can be read by scanner!!', WQM_Common::PLUGIN_SYSTEM_NAME ) ?></p></div>
        </td>
    </tr>
    <tr class="field-filename">
        <th><label for="field-filename"><?php _e( 'File name', WQM_Common::PLUGIN_SYSTEM_NAME ) ?></label></th>
        <td>
            <input type="text" name="wqm_filename" id="field-filename" class="regular-text" value="<?php esc_attr_e( $wqm_filename ); ?>">
            <span class="description"><?php _e( 'File name when enable save/open v-card as file in widget. Enabled patterns: ', WQM_Common::PLUGIN_SYSTEM_NAME ) ?><span class="patrn">%_name_%</span>, <span class="patrn">%_nickname_%</span>, <span class="patrn">%_title_%</span>, <span class="patrn">%_organization_%</span></span>
        </td>
    </tr>
    </tbody>
</table>
