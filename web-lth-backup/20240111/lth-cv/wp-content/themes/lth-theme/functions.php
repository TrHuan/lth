...
// addons
if( have_rows('other', 'option') ):
	while( have_rows('other', 'option') ): the_row(); 		
		if( have_rows('addons') ):
			while( have_rows('addons') ): the_row(); 
				define('ADDONS_DIR', LIBS_DIR . '/addons');

				$projects_management = get_sub_field('projects_management');

				if ($projects_management == 'yes') {
					require_once(ADDONS_DIR . '/quan-ly-du-an.php');
				}
			endwhile;
		endif;
	endwhile;
