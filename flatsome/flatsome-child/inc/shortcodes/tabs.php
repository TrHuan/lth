...
function remove_ux_tabgroup_theme_shortcodes() {
	...
	add_shortcode('tabgroup', 'gco_ux_tabgroup');
	add_shortcode('tabgroup_vertical', 'gco_ux_tabgroup');
	add_shortcode('tab', 'gco_ux_tab' );

}
add_action('init', 'remove_ux_tabgroup_theme_shortcodes');

// [tabgroup]
function gco_ux_tabgroup( $params, $content = null, $tag = '' ) {
	$GLOBALS['tabs'] = array();
	$GLOBALS['tab_count'] = 0;
	$i = 1;
  ...
  if ( $visibility ) $wrapper_class[] = $visibility;
	...
	if( is_array( $GLOBALS['tabs'] )){
		...
		foreach( $GLOBALS['tabs'] as $key => $tab ){
			...
			if($title) $title = '<h2 class="uppercase text-'.$align.'">'.$title.'</h2>';
			$return = '
...
function gco_ux_tab( $params, $content = null) {
	extract(shortcode_atts(array(
			'title' => '',
			'title_small' => ''
	), $params));
