...
<?php if ( get_theme_mod( 'blog_single_footer_meta', 1 ) ) : ?>
	<footer class="entry-meta text-<?php echo get_theme_mod( 'blog_posts_title_align', 'center' ); ?>">
		...
		if ( '' != $tag_list ) {
			$meta_text = __( 'Mục nhập này đã được đăng trong: %1$s và được gắn thẻ %2$s.', 'flatsome' );
		} else {
			$meta_text = __( 'Mục nhập này đã được đăng trong: %1$s. Đánh dấu trang <a href="%3$s" title="Permalink to %4$s" rel="bookmark">liên kết cố định</a>.', 'flatsome' );
		}
